...
<?php
	...
	if(isset($_SESSION['Sesion']) && $_SESSION['Sesion']['id_usuario']!="")
	{
		...
		$id_RutinaCliente = ""; // Se define en blanco para verificar si es mandado desde clientes_rutina o es rutina nueva.
		...
		//Si está definido el $_GET['id_cliente'] es por que acceden a el por el apartado del menú de clientes_rutina.php
		if(isset($_GET['id_cliente']))
		{
			$id_cliente		  = $_GET['id_cliente'];
			//Buscar los datos de la rutina clientes, tabla sg_rutinaclientes con el id_cliente
			$ResultCliente	  = $consultar->_ConsultarRutinaClientePorIdCliente($id_cliente);
			$filaCliente   	  = $ResultCliente->fetch_assoc();
		 	$id_RutinaCliente = $filaCliente['id_rutinaCliente'];
		}//if
		
		if(isset($_GET['id_Rutina']) || $id_RutinaCliente!= "")
		{
			//$id_Rutina 		= $_GET['id_Rutina'];	
			$id_Rutina		= ($id_RutinaCliente!="")?$id_RutinaCliente:$_GET['id_Rutina'];
			...
			for($i=0; $i<$num_ejercicios; $i++)
			{
				...
				$Ejercicio = array("id_rutinaCliente"=>$fila["id_rutinaCliente"], "nb_ejercicio"=>$fila['nb_ejercicio'], 
...
$(document).ready(function(){
	...
	$("#Continuar_Rutina").click(function(){
		...
		if(confirmacion == true)
		{
			...
			window.location='index.php?nav=RutinasPreClie&Rutina='+id_Rutina; //Elegir rutinas porm día
		}
	});//Continuar_Rutina
...
function CambiarDia(id_par,dia_semana)
{
	//Obtener el tr que está encima
		...
		if (id_Hijo == 0)
		{
			...
			Actividad_Sola			 = 0; //Es que solo existe una actividad en ese día asignado.
			MismoLugar_Hijo 		 = 0; 
			MismoLugar_Padre 	 	 = 0;
...
function ActualizaDia(dia)
{
	...
	//alert(id_Sustituye);
	$.post( "modulos/Clientes/DiasEjercicioCargar.php", { Params: Params})
