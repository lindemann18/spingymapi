<?php
	...
	if(isset($_SESSION['Sesion']) && $_SESSION['Sesion']['id_usuario']!="")
	{
		if(isset($_GET['id']))
		{
			...
			$consultar=new Consultar();
			$result=$consultar->_ConsultarMusculosPorId($id);
...
.Botones {margin-right: 4%;}
            ...
            <aside class="right-side">
                ...
                <section class="content">
                    ...
                    <div class="alert alert-success alert-dismissible" role="alert" style="display:none;" id="MusculoEditadoNotificacion">
  <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
  <strong>Bien Hecho!</strong> M&uacute;sculo Editado.</div>
							...
							<div class="box box-primary">
                                ...
                                <div class="box-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="nb_musculo" id="nb_musculo" campo="Nombres" class="form-control requerido" 
                                        placeholder="Nombre de M&uacute;sculo" value="<?php echo $fila['nb_musculo']?>">
                                        <input type="hidden" id="id_musculo" value="<?php echo $fila['id'];?>">
                                    </div>
                                    
                                    <div class="form-group">
                                        <label for="nb_apellidos">Descripci&oacute;n</label>
                                        <textarea name="desc_musculo" id="desc_musculo" class="form-control requerido"><?php echo $fila['desc_musculo'];?></textarea>
                                    </div>
                                     ...
                                     <div class="form-group">
										...
										<select name="id_TipoRutina" id="id_TipoRutina" class="form-control requerido">
                                            ...
                                            <?php 
												while($filaRutina=$ResultTiposRutina->fetch_assoc())
												{
													if($fila['id_TipoRutina']==$filaRutina['id'])
													{
														echo'<option value="'.$filaRutina['id'].'" selected>'.$filaRutina['nb_TipoRutina'].'</option>';
													}else {echo'<option value="'.$filaRutina['id'].'">'.$filaRutina['nb_TipoRutina'].'</option>';}
												}//while
											?>
                                        </select> 
                                    </div>
