<?php
	...
	if(isset($_SESSION['Sesion']) && $_SESSION['Sesion']['id_usuario']!="")
	{
		...
		if(isset($_GET['id']))
		{
			...
			$consultar=new Consultar();
			$result=$consultar->_ConsultarEjercicioPorId($id);
			$fila=$result->fetch_assoc();
			$ResultTipRut  = $consultar->_ConsultarTiposRutinaPorMusculo($fila['id_Musculo']); //Tipos de rutina
			$num_rowsTipRut = $ResultTipRut->num_rows; //rows de los tipos de rutina existentes
			$ResultMaqPorRut = $consultar->_ConsultarMaquinaPorCategoria($fila['id_CategoriaMaquina']);
			$num_maqRut      = $ResultMaqPorRut->num_rows;
			//Consultar las máquinas relacionadas con el músculo que viene el registro
			//$resultMaquinas=$consultar->_ConsultarMaquinaPorMusculo($fila['id_Musculo']);
			
			//Consultar Tipo de rutina Por Musculo
			$resultRutinas=$consultar->_BuscarRutinasPorMusculo($fila['id_Musculo']);
		}//if
...
.Botones {margin-right: 4%;}
            ...
            <aside class="right-side">
                ...
                <section class="content">
                    ...
                    <div class="alert alert-success alert-dismissible" role="alert" style="display:none;" id="EjercicioEditadoNotificacion">
                    	...
                    	<div class="col-md-8 pull-right Cuestionario">
							...
							<div class="box box-primary">
                                ...
                                <div class="box-body">
                                    ...
                                    <input type="hidden" id="id_Ejercicio" value="<?php echo $fila['id']?>">
                                    <div class="form-group">
                                        <label for="nb_cliente">Nombre </label>
                                        <input type="text" name="nb_ejercicio" id="nb_ejercicio" campo="Nombres"
                                         class="form-control requerido" placeholder="Nombre del Ejercicio" value="<?php echo $fila['nb_ejercicio']?>">
                                    </div>
                                    
                                    <div class="form-group">
                                        <label for="nb_apellidos">Descripci&oacute;n</label>
                                        <textarea name="desc_ejercicio" id="desc_ejercicio" 
                                        class="form-control requerido"><?php echo $fila['desc_ejercicio']?></textarea>
                                    </div>
                                    ...
                                    <div class="form-group">
                                       ...
                                       <select name="id_musculo" id="id_musculo" class="form-control requerido" onChange="TraertiposRutina()">
                                        ...
                                        <?php 
											...
											{
												if($fila['id_Musculo']==$filaMusculo['id'])
												{
													echo '<option value="'.$filaMusculo['id'].'" selected>'.$filaMusculo['nb_musculo'].'</option>';	
												}//if
												else{echo '<option value="'.$filaMusculo['id'].'">'.$filaMusculo['nb_musculo'].'</option>';}
											}//while
                                    ...
                                    <div class="form-group">
                                       ...
                                       <select name="id_CategoriaMaquina" id="id_CategoriaMaquina" class="form-control requerido"
                                        onChange="TraerMaquinasPorCategoria()">
                                       	<option value="">Seleccionar...</option>
											...
											{
												if($fila['id_CategoriaMaquina']==$filaMaquina['id'])
												{
													echo '<option value="'.$filaMaquina['id'].'" selected>
															'.$filaMaquina['nb_CategoriaMaquina'].'
														  </option>';	
												}
												else 
												{
                                     ...
                                     <div class="form-group">
                                       ...
                                       <select name="id_maquina" id="id_maquina" class="form-control requerido" onChange="">
                                       	<option value="">Seleccionar...</option>
                                       	<?php 
											while ($filaMaquinas = $ResultMaqPorRut->fetch_assoc())
											{
												if($filaMaquinas['id']==$fila['id_Maquina'])
												{
													echo '<option value="'.$filaMaquinas['id'].'" selected>
															'.$filaMaquinas['nb_maquina'].'
														  </option>';	
												}
												else 
												{
													echo '<option value="'.$filaMaquinas['id'].'">'.$filaMaquinas['nb_maquina'].'</option>';
												}
											}//while
										?>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <?php 
											while($filaTiposRutina= $ResultTipRut->fetch_assoc())
											{
												if($fila['id_TipoRutina']==$filaTiposRutina['id'])
												{
													echo '<option value="'.$filaTiposRutina['id'].'" selected>
													'.$filaTiposRutina['nb_TipoRutina'].'</option>';	
												}//if
												else
												{
													echo '<option value="'.$filaTiposRutina['id'].'">
														'.$filaTiposRutina['nb_TipoRutina'].'
														 </option>';
												}
											}//while
										?>
...
function RegistraMaquina()
{
	...
	Arr['id_Ejercicio']		= $("#id_Ejercicio").val();
	Arr['Accion']			="EditarEjercicio";
	...
	console.log(Params);
		$.ajax({
				...
				success: function(response){
					...
					var objJSON = eval("(function(){return " + response + ";})()");
					$("#EjercicioEditadoNotificacion").css("display","inherit");
					$("#EjercicioEditadoNotificacion").delay( 8000 ).fadeOut();
