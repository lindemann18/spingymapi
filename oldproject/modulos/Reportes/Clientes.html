                                    ...
                                    <table id="listados" class="table table-bordered table-striped">
                                        ...
                                        while($fila=$result->fetch_assoc()):
                                            ...
                                            <tr onclick="SeleccioinarDato('.$fila['id_cliente'].')" id=fila"'.$fila['id_cliente'].'">
												...
												<td>'.$fila['Cliente_Apellidos'].'</td>
                                                <td class="text-center">'.$fila['Cantidad_Biotest'].'</td>
                                                <td>'.$fila['Entrenador_nombre']." ".$fila['Entrenador_Apellidos'].'</td>
												<td>'.$fila['fh_biotest'].'</td>
                                            </tr>
											');	
...
$(document).ready(function(){
	...
	$("td sorting_asc").removeClass("sorting_asc");
});//document ready
...
function ReportePdf()
{
	var confirmacion = confirm('¿Está seguro de Imprimir este Reporte?');
		if(confirmacion == true){
				...
				window.open('modulos/Reportes/ClientesPdf.php');
...
function EnviarReportePdf()
{
	...
	$.ajax("modulos/Reportes/ClientesPdf.php?Mail=1").then(function(response)
	{	
...
function LoadTable()
{
	//CREA LA TABLA EN FORMATO BOOTSTRAP DATATABLE
				tabla = $('#listados').dataTable( {
					"aaSorting": [[3,'desc']],						//ORDENA DE FORMA DESCENDENTE LA PRIMER COLUMNA (RECORDAR QUE EN LOS ARREGLOS SE COMIENZA DESDE CERO [0] )
